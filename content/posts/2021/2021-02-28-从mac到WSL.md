...
1. wsl的服务起起来之后,在windows的浏览器只能使用localhost才能访问,这个是wsl2的实现协议导致的,目前官网也没有很好的解决方案,可以使用一个脚本来批量处理:
    ...
    >https://github.com/microsoft/WSL/issues/4150  
1. 在wsl启动的服务有些端口提示冲突的问题,主要是3000与3306等端口都默认被windows的一个动态端口协议占用的
    (官方说明从winserver2008之后包括win10都默认修改了,但是安装的系统还是老的,需要手动执行下更新)
    还是万能的github之后可以通过命令修改:
    ```shell
    netsh int ipv set dynamic tcp start=49152 num=16384
    ```

    参考链接:
    >https://github.com/docker/for-win/issues/3171#issuecomment-736155035
